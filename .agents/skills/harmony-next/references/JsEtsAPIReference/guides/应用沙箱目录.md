...
## 调试时定位沙箱文件
模拟器或真机调试时，`/data/storage/...` 是应用进程看到的沙箱路径；`hdc shell` 看到的通常是真实物理路径。不要在代码中硬编码物理路径，应通过 `Context.filesDir`、`Context.cacheDir`、`Context.databaseDir` 等属性获取应用文件路径。

常用只读定位命令：

```bash
hdc -t <target> shell aa dump -l
hdc -t <target> shell ls /data/app/el2/100/base/<bundleName>
hdc -t <target> shell ls /data/app/el2/100/base/<bundleName>/haps/entry/files
hdc -t <target> shell ls /data/app/el2/100/database/<bundleName>
```

需要取回文件时再执行：

```bash
hdc -t <target> file recv /data/app/el2/100/base/<bundleName>/haps/entry/files ./files
```
