...
def should_skip(path: Path) -> bool:
    ...
    return "__pycache__" in parts or ".DS_Store" in parts or path.name.endswith(".pyc")
...
def package_skill(repo_root: Path, skill_root: Path, output: Path, release_tag: str | None = None) -> dict[str, object]:
    ...
    with zipfile.ZipFile(output, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        for path in sorted(skill_root.rglob("*")):
            if not path.is_file() or should_skip(path.relative_to(skill_root)):
                continue
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Package harmony-next skill with build metadata.")
    parser.add_argument("--repo-root", type=Path, default=Path(__file__).resolve().parents[2])
    parser.add_argument("--skill-root", type=Path, default=Path(__file__).resolve().parents[1])
    parser.add_argument("--output", type=Path, default=Path("harmony-next.skill.zip"))
